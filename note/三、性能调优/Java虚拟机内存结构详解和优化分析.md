...
- 加载： 将.class文件（全限定名 包名 + 类名）从磁盘读取到内存
- 验证：验证字节码的正确性
- 准备：给类的静态变量发呢配内存并赋予默认值
- 解析：类装载器所引用的其它所有类
...
# 双亲委派机制（父类委派模型）
当加载一个类时 一般会先委托给父类加载机制直到顶层，由于加载器的特性会将加载任务交给对应的加载器来进行加载
目的：防止用户随意定义类，篡改核心类库，避免类的重复加载
...
# 垃圾收集器（JDK1.8使用的Parallel并行收集器）STW：停止所有的用户线程来跑垃圾收集
- Serial：停顿时间比较长
